    ...
    def __init_mosi(self):
        ...
        if self.mode == 'train':
            ...
            if isinstance(self.missing_rate_eval_test, dict):
                ...
                self.labels['missing_rate_l'] = np.zeros(self.num_samples) if t_modality else np.ones(self.num_samples)
                self.labels['missing_rate_a'] = np.zeros(self.num_samples) if a_modality else np.ones(self.num_samples)
                self.labels['missing_rate_v'] = np.zeros(self.num_samples) if v_modality else np.ones(self.num_samples)
            else:
