...
parser = argparse.ArgumentParser(
    description='R-PMF训练脚本，支持命令行参数覆盖配置文件',
    epilog='''
示例:
  python train.py --config_file configs/train_mosi.yaml
  python train.py --config_file configs/train_mosi.yaml --base.n_epochs=1
  python train.py --config_file configs/train_mosi.yaml --base.lr=0.0001 --base.n_epochs=50
'''
)
...
def train(model, train_loader, optimizer, loss_fn, epoch, metrics):
    ...
    for cur_iter, data in enumerate(train_loader):
        ...
        loss['loss'].backward()
        # Gradient clipping to prevent gradient explosion (standard practice)
