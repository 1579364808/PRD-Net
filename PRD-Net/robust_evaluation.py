...
def main():
    ...
    for key_path, value in override_args.items():
        set_nested_value(args, key_path, value)
    ...
    key_eval = args.get('base', {}).get('key_eval', opt.key_eval) if opt.key_eval == '' else opt.key_eval

    print("Using PRD-Net model")
    ...
    for cur_r in missing_rate_list:
        ...
        if dataset_name == 'sims':
            for _, cur_seed  in enumerate([1111, 1112, 1113]):
                if opt.ckpt_dir:
                    best_ckpt = os.path.join(opt.ckpt_dir, f'best_{key_eval}_{cur_seed}.pth')
                else:
